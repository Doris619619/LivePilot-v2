...
export type ObsInstance = { id: "local"; scene: "LIVE"; video: "VIDEO"; music: "MUSIC" };
export const instance: ObsInstance = { id: "local", scene: "LIVE", video: "VIDEO", music: "MUSIC" };
export class ObsController {
  ...
  async validate() {
    ...
    if (!scenes.scenes.some(s => s.sceneName === instance.scene)) throw new AppError("OBS_SCENE", "OBS 缺少 LIVE 场景。请按 README 创建 LIVE 和两个媒体源 VIDEO / MUSIC。");
    const items = await this.call("GetSceneItemList", { sceneName: instance.scene });
    for (const name of [instance.video, instance.music]) {
  ...
  async configureMedia(video: string, music: string, videoAudio: boolean) {
    for (const [inputName, file] of [[instance.video, video], [instance.music, music]]) {
      ...
      const item = await this.call("GetSceneItemId", { sceneName: instance.scene, sourceName: inputName });
      await this.call("SetSceneItemEnabled", { sceneName: instance.scene, sceneItemId: item.sceneItemId, sceneItemEnabled: true });
    }
    await this.call("SetInputMute", { inputName: instance.video, inputMuted: !videoAudio });
    await this.call("SetInputMute", { inputName: instance.music, inputMuted: false });
    await this.call("SetCurrentProgramScene", { sceneName: instance.scene });
    for (const inputName of [instance.video, instance.music]) {
